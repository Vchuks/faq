...
//   const getPix = Math.floor(Math.random() * 100);
//   console.log(getPix);
// }
// console.log(handleClick());

function handleClick() {
  const getId = Math.floor(Math.random() * 4) + 1;
  console.log(getId);
  const getPix = faqList.find((pix) => {
    if (pix.id === getId && pix.id != 0) {
      console.log(pix.title);
    }
  });
}
console.log(handleClick());
